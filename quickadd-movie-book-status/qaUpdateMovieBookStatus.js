...
async function isComplete(config){
    ...
    // Check if started is set, if not we assume the book was started and finished on the same day
    let start = "";
    try {
        start = await MetaEdit.api.getPropertyValue(config.startDate, config.activeFile);
    } catch (e) {
        NotifyUserAndLog(`Unable to locate key '${config.startDate}'.`);
    }
    ...
    if (!start){
        try {
            ...
            NotifyUserAndLog(`Unable to update key '${config.startDate}'.`);
        }
    }
    ...
    try {
        ...
        NotifyUserAndLog(`Unable to update key '${config.completedDate}'.`);
    }
    ...
    if (TYPES_OF_READING.includes(config.type)){
        try {
            const total = await MetaEdit.api.getPropertyValue(config.totalPage, config.activeFile);
